 ...
 * \ingroup compnt
 * @{
 * \defgroup conflict Flow Rule Conflict Detection
 * \brief (Security) flow rule conflict detection
 * @{
 */
...
{
    flow_t list[__CONFLICT_MATCH_LIST_SIZE];
    ...
    for (i=0; i<num_actions; i++) {
        ...
        if (type == ACTION_OUTPUT) {
            ...
            for (j=0; j<(2 * (i + 1)); j++, curr++) {
                ...
                if ((j % 2) != 0)
                    list[curr].match.vlan_id = rule->action[i].vlan_id;
            }
        } else if (type == ACTION_SET_VLAN_PCP) {
            for (j=0; j<(2 * (i + 1)); j++, curr++) {
                memmove(&list[curr], &list[curr/2], sizeof(flow_t));
                if ((j % 2) != 0)
                    list[curr].match.vlan_pcp = rule->action[i].vlan_pcp;
            }
        } else if (type == ACTION_SET_SRC_MAC) {
            for (j=0; j<(2 * (i + 1)); j++, curr++) {
                memmove(&list[curr], &list[curr/2], sizeof(flow_t));
                if ((j % 2) != 0)
                    memmove(&list[curr].match.src_mac, rule->action[i].mac_addr, ETH_ALEN);
            }
        } else if (type == ACTION_SET_DST_MAC) {
            for (j=0; j<(2 * (i + 1)); j++, curr++) {
                memmove(&list[curr], &list[curr/2], sizeof(flow_t));
                if ((j % 2) != 0)
                    memmove(&list[curr].match.dst_mac, rule->action[i].mac_addr, ETH_ALEN);
            }
        } else if (type == ACTION_SET_SRC_IP) {
            for (j=0; j<(2 * (i + 1)); j++, curr++) {
                memmove(&list[curr], &list[curr/2], sizeof(flow_t));
                if ((j % 2) != 0)
                    list[curr].match.src_ip = rule->action[i].ip_addr;
            }
        } else if (type == ACTION_SET_DST_IP) {
            for (j=0; j<(2 * (i + 1)); j++, curr++) {
                memmove(&list[curr], &list[curr/2], sizeof(flow_t));
                if ((j % 2) != 0)
                    list[curr].match.dst_ip = rule->action[i].ip_addr;
            }
        } else if (type == ACTION_SET_IP_TOS) {
            for (j=0; j<(2 * (i + 1)); j++, curr++) {
                memmove(&list[curr], &list[curr/2], sizeof(flow_t));
                if ((j % 2) != 0)
                    list[curr].match.ip_tos = rule->action[i].ip_tos;
            }
        } else if (type == ACTION_SET_SRC_PORT) {
            for (j=0; j<(2 * (i + 1)); j++, curr++) {
                memmove(&list[curr], &list[curr/2], sizeof(flow_t));
                if ((j % 2) != 0)
                    list[curr].match.src_port = rule->action[i].port;
            }
        } else if (type == ACTION_SET_DST_PORT) {
            for (j=0; j<(2 * (i + 1)); j++, curr++) {
                memmove(&list[curr], &list[curr/2], sizeof(flow_t));
                if ((j % 2) != 0)
                    list[curr].match.dst_port = rule->action[i].port;
            }
        }
    }
...
{
    LOG_INFO(CONFLICT_ID, "Init - Flow rule conflict detection");

    num_rules = 0;

    rule_table = (rule_table_t *)CALLOC(__DEFAULT_TABLE_SIZE, sizeof(rule_table_t));
    if (rule_table == NULL) {
        LOG_ERROR(CONFLICT_ID, "calloc() failed");
        return -1;
    }
...
{
    LOG_INFO(CONFLICT_ID, "Clean up - Flow rule conflict detection");
