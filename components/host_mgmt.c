...
int host_mgmt_handler(const event_t *ev, event_out_t *ev_out)
{
    ...
    case EV_HOST_DELETED:
        ...
        break;
    case EV_HOST_GET_INFO:
        PRINT_EV("EV_HOST_GET_INFO\n");
        {
            host_t *host = ev_out->host_data;
            ...
            for (i=0; i<__DEFAULT_TABLE_SIZE; i++) {
                ...
                host_t *curr = host_table[i].head;
                while (curr != NULL) {
                    if (curr->ip == host->ip || curr->mac == host->mac) {
                        memmove(host, curr, sizeof(host_t));
                        host->prev = host->next = host->r_next = NULL;
                        stop = TRUE;
                        break;
                    }
        ...
        break;
    case EV_HOST_GET_ALL_INFO:
        PRINT_EV("EV_HOST_GET_ALL_INFO\n");
        {
            host_t *host = ev_out->host_data;

            host = (host_t *)MALLOC(sizeof(host_t) * num_hosts);
            if (host == NULL) {
                break;
            }
            ...
            for (i=0; i<__DEFAULT_TABLE_SIZE; i++) {
                ...
                host_t *curr = host_table[i].head;
                while (curr != NULL) {
                    memmove(&host[idx], curr, sizeof(host_t));

                    if (idx == 0) {
                        host[idx].prev = NULL;

                        if (num_hosts > 1) {
                            host[idx].next = &host[idx + 1];
                        } else {
                            host[idx].next = NULL;
                        }
                    } else {
                        host[idx].prev = &host[idx - 1];
