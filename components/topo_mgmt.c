...
int topo_mgmt_handler(const event_t *ev, event_out_t *ev_out)
{
    ...
    case EV_LINK_DELETED:
        ...
        break;
    case EV_LINK_GET_INFO:
        PRINT_EV("EV_LINK_GET_INFO\n");
        {
            port_t *link = ev_out->port_data;

            link = (port_t *)MALLOC(sizeof(port_t) * num_links);
            if (link == NULL) {
                break;
            }

            memset(link, 0, sizeof(port_t) * num_links);
            ...
            for (i=0; i<__DEFAULT_TABLE_SIZE; i++) {
                if (topo[i].dpid == link->dpid) { // check source dpid
                    ...
                    for (j=0; j<__MAX_NUM_PORTS; j++) {
                        if (topo[i].port[j].port > 0) { //check source port
                            if (topo[i].port[j].target_port > 0) { // check destination port
                                link[idx].dpid = topo[i].dpid;
                                link[idx].port = topo[i].port[j].port;
                                link[idx].target_dpid = topo[i].port[j].target_dpid;
                                link[idx].target_port = topo[i].port[j].target_port;

                                link[idx].rx_packets = topo[i].port[j].rx_packets;
                                link[idx].rx_bytes = topo[i].port[j].rx_bytes;
                                link[idx].tx_packets = topo[i].port[j].tx_packets;
                                link[idx].tx_bytes = topo[i].port[j].tx_bytes;

                                if (idx == 0) {
                                    link[idx].prev = NULL;

                                    if (num_links > 1) {
                                        link[idx].next = &link[idx + 1];
                                    } else {
                                        link[idx].next = NULL;
                                    }
                                } else {
                                    link[idx].prev = &link[idx - 1];
        ...
        break;
    case EV_LINK_GET_ALL_INFO:
        PRINT_EV("EV_LINK_GET_ALL_INFO\n");
        {
            port_t *link = ev_out->port_data;
