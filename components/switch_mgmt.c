...
int switch_mgmt_handler(const event_t *ev, event_out_t *ev_out)
{
    ...
    case EV_SW_GET_INFO:
        PRINT_EV("EV_SW_GET_INFO\n");
        {
            ...
            while (curr != NULL) {
                if (curr->dpid == sw->dpid) {
                    memmove(sw, curr, sizeof(switch_t));
                    sw->port = sw->fd = sw->xid = 0;
                    sw->prev = sw->next = NULL;
                    break;
                }
                curr = curr->next;
            }
        ...
        break;
    case EV_SW_GET_ALL_INFO:
        PRINT_EV("EV_SW_GET_ALL_INFO\n");
        {
            switch_t *sw = ev_out->sw_data;

            sw = (switch_t *)MALLOC(sizeof(switch_t) * num_switches);
            if (sw == NULL) {
                break;
            }
            ...
            while (curr != NULL) {
                memmove(&sw[idx], curr, sizeof(switch_t));
                sw->port = sw->fd = sw->xid = 0;
                ...
                if (idx == 0) {
                    ...
                    if (num_switches > 1) {
                        sw[idx].next = &sw[idx + 1];
                    } else {
                        sw[idx].next = NULL;
                    }
                } else {
                    sw[idx].prev = &sw[idx - 1];
