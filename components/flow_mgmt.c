...
int flow_mgmt_handler(const event_t *ev, event_out_t *ev_out)
{
    ...
    case EV_FLOW_DELETED:
        ...
        break;
    case EV_FLOW_GET_INFO:
        PRINT_EV("EV_FLOW_GET_INFO\n");
        {
            flow_t *flow = ev_out->flow_data;

            uint64_t dpid = flow->dpid;

            flow = (flow_t *)MALLOC(sizeof(flow_t) * num_flows);
            if (flow == NULL) {
                break;
            }
            ...
            for (i=0; i<__DEFAULT_TABLE_SIZE; i++) {
                ...
                while (curr != NULL) {
                    if (curr->dpid == dpid) {
                        memmove(&flow[idx], curr, sizeof(flow_t));
                        if (idx == 0) {
                            flow[idx].prev = NULL;

                            if (num_flows > 1) {
                                flow[idx].next = &flow[idx + 1];
                            } else {
                                flow[idx].next = NULL;
                            }
                        } else {
                            flow[idx].prev = &flow[idx - 1];
        ...
        break;
    case EV_FLOW_GET_ALL_INFO:
        PRINT_EV("EV_FLOW_GET_ALL_INFO\n");
        {
            flow_t *flow = ev_out->flow_data;
