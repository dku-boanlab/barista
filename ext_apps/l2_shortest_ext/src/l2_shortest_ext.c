 ...
 * \ingroup app
 * @{
 * \defgroup l2_shortest_ext External L2 Shortest-Path
 * \brief (Network) external L2 shortest path
 * @{
 */
...
#include "l2_shortest_ext.h"

/** \brief L2 shortest path ID */
#define L2_SHORTEST_EXT_ID 123970670
...
static int add_switch(const switch_t *sw)
{
    ...
    ALOG_INFO(L2_SHORTEST_EXT_ID, "Recalculated shortest-paths");
...
static int send_packet(const pktin_t *pktin, uint16_t port)
{
    ...
    av_dp_send_packet(L2_SHORTEST_EXT_ID, &out);
...
static int insert_flow(const pktin_t *pktin, uint16_t port)
{
    ...
    av_dp_insert_flow(L2_SHORTEST_EXT_ID, &out);
 ...
 */
int l2_shortest_ext_main(int *activated, int argc, char **argv)
{
    ALOG_INFO(L2_SHORTEST_EXT_ID, "Init - external L2 shortest-path");
 ...
 * \param activated The activation flag of this application
 */
int l2_shortest_ext_cleanup(int *activated)
{
    ALOG_INFO(L2_SHORTEST_EXT_ID, "Clean up - external L2 shortest-path");
 ...
 */
int l2_shortest_ext_cli(cli_t *cli, char **args)
{
 ...
 */
int l2_shortest_ext_handler(const app_event_t *av, app_event_out_t *av_out)
{
    switch (av->type) {
