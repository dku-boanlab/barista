 ...
 * \ingroup app
 * @{
 * \defgroup l2_learning_ext External L2 Learning
 * \brief (Network) external L2 learning
 * @{
 */
...
#include "l2_learning_ext.h"

/** \brief L2 learning ID */
#define L2_LEARNING_EXT_ID 153576300
...
static int send_packet(const pktin_t *pktin, uint16_t port)
{
    ...
    av_dp_send_packet(L2_LEARNING_EXT_ID, &out);
...
static int insert_flow(const pktin_t *pktin, uint16_t port)
{
    ...
    av_dp_insert_flow(L2_LEARNING_EXT_ID, &out);
 ...
 */
int l2_learning_ext_main(int *activated, int argc, char **argv)
{
    ALOG_INFO(L2_LEARNING_EXT_ID, "Init - external L2 learning");
 ...
 * \param activated The activation flag of this application
 */
int l2_learning_ext_cleanup(int *activated)
{
    ALOG_INFO(L2_LEARNING_EXT_ID, "Clean up - external L2 learning");
 ...
 */
int l2_learning_ext_cli(cli_t *cli, char **args)
{
 ...
 */
int l2_learning_ext_handler(const app_event_t *av, app_event_out_t *av_out)
{
    switch (av->type) {
